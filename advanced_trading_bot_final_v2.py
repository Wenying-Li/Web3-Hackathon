...
class RoostooV3Client:
    ...
    def _signed_request(self, method, endpoint, params=None, data=None):
        ...
        try:
            ...
            try:
                ...
                logging.debug(f"非JSON响应: {response.text}")
                return None
    ...
    def place_order(self, coin, side, quantity, price=None):
        ...
        res = self._signed_request("POST", "/v3/place_order", data=data)
        logging.info(f"下单接口返回: {json.dumps(res, default=str)}")
        return res
...
def compute_total_value_from_wallet(wallet, ticker_map):
    total = 0.0
    if not isinstance(wallet, dict):
        return 0.0
    usd_free = safe_float(wallet.get("USD", {}).get("Free", 0))
    total += usd_free
    for coin, data in wallet.items():
        ...
        try:
            ...
            if pair in ticker_map and ticker_map[pair].get("LastPrice", 0) > 0:
                price = safe_float(ticker_map[pair].get("LastPrice", 0))
                quantity = safe_float(data.get("Free", 0)) + safe_float(data.get("Lock", 0))
...
# ---
# 主策略逻辑（主要改动：记录下单结果并在下单后刷新并打印余额总值）
# ---
def run_strategy():
    ...
    total_portfolio_value = compute_total_value_from_wallet(wallet, ticker_map)
    current_holdings = {}
    for coin, data in wallet.items():
        if coin == "USD": continue
        pair = f"{coin}/USD"
        if pair in ticker_map and ticker_map[pair].get("LastPrice", 0) > 0:
            price = safe_float(ticker_map[pair]["LastPrice"], 0)
            quantity = safe_float(data.get("Free", 0)) + safe_float(data.get("Lock", 0))
    ...
    for pair in all_pairs_in_play:
        ...
        if trade_value < 0:
            ...
            order_result = client.place_order(coin, "SELL", sell_quantity)
    ...
    # 下单后刷新余额并记录变化（便于看到总资产是否发生变化）
    balance_after = client.get_balance()
    if balance_after and balance_after.get("Success"):
        wallet_after = balance_after.get("Wallet") or balance_after.get("SpotWallet") or {}
        total_after = compute_total_value_from_wallet(wallet_after, ticker_map)
        logging.info(f"下单后总资产估算: ${total_after:.2f} （下单前: ${total_portfolio_value:.2f}）")
    else:
        logging.warning("下单后无法获取或解析到账户余额。")
