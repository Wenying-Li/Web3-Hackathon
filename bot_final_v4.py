...
def adjust_quantity_by_step(quantity, step_size_str):
    ...
    # 计算 quantity 是 step_size 的多少倍, 然后向下取整
    multiplier = math.floor(quantity / step_size)
    # 重新计算调整后的 quantity
    adjusted_quantity = multiplier * step_size
    
    # 为了避免浮点数精度问题, 格式化为字符串再转回浮点数
    # 首先确定 step_size 的小数位数
    if '.' in step_size_str:
...
def run_strategy():
    ...
    if not ex_info or not ex_info.get("Success"):
        logging.error("无法获取交易所信息 (exchangeInfo), 策略中止。")
        return
    # 将交易对规则存入一个字典方便查找
    symbols_info = {s['pair']: s for s in ex_info.get('symbols', [])}
